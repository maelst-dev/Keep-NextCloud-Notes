...
- **Checklist strikethrough normalization (`MarkwonFactory.formatChecklistStrikethrough`)**: Before rendering Markdown in either cards or editor preview, run `formatChecklistStrikethrough`. This safely ensures every `- [x]` item has matched `~~text~~` (closing any unclosed/dangling tildes and adding tildes for tasks synced from Nextcloud Web).
- **Card excerpt preservation (`NoteEntity.excerpt`)**: Never use arbitrary character truncation (like `take(200)`) on Markdown content, as it cuts off mid-line and breaks Markdown syntax (such as dangling `~~`). Instead, take complete lines (up to 10 lines) and let `TextView` with `maxLines="8"` and `ellipsize="end"` smoothly handle layout clipping.
...
- Unit tests are located in `app/src/test/java/com/keepnc/ui/editor/EditorViewModelTest.kt`, `NotesRepositoryTest.kt`, and `AuthInterceptorTest.kt` using `kotlinx-coroutines-test` and JUnit 4.
- `TokenStorage` provides a no-arg constructor for unit tests without Android Context.

### 9. Sync Concurrency & Deduplication
- **Mutex locking**: `NotesRepository.syncWithServer()` MUST use a `Mutex` (`syncMutex.withLock`) to prevent race conditions when UI (`NotesViewModel`) and WorkManager (`SyncWorker`) trigger sync concurrently on app startup.
- **Local deduplication**: Step 3 of sync checks all local entries matching `serverId` via `noteDao.getNotesByServerId(dto.id)`. If duplicate rows exist in Room, it keeps and updates the primary one and deletes all duplicate entries.
- **Auto Backup**: `android:allowBackup="false"` in `AndroidManifest.xml` prevents Android Auto Backup from restoring a stale Room database after app reinstall when Keystore encryption keys are discarded.
- **Session clean slate**: Room notes are cleared via `repository.clearAllLocalNotes()` on logout (`MainActivity.logout`) and on login (`LoginViewModel`).

### 10. Dynamic Server URL & AuthInterceptor
- **Retrofit placeholder**: The placeholder URL in `AppModule.kt` must always include the full Nextcloud Notes API prefix: `"https://placeholder.invalid/index.php/apps/notes/api/v1/"`.
- **URL rewrite**: `AuthInterceptor` dynamically rewrites requests to the stored server URL using `v1` index extraction with a safe fallback, ensuring subpaths like `/notes` are never stripped (preventing HTTP 400 Bad Request on initial sync).

### 11. ConstraintLayout Group & Lifecycle Resumption in Editor
- **Do not use `Group` for mutually exclusive mode views**: Never use `androidx.constraintlayout.widget.Group` to bind containers that toggle visibility conditionally (such as `scroll_content` vs `scroll_preview`).
- **Reason**: When the app is minimized and restored, `repeatOnLifecycle(Lifecycle.State.STARTED)` re-emits `EditorUiState.Editing`. Any `group.visibility = View.VISIBLE` call forcibly resets all its referenced IDs to `VISIBLE`, overriding `scroll_content.visibility = View.GONE` and causing raw text and Markdown preview to draw on top of each other simultaneously.
- **Centralized mode control**: Mode switching must always be driven via `EditorFragment.applyMode(preview: Boolean)` without helper `Group` views interfering with visibility.

### 12. Checklist Rendering & Size Constraints (Cards vs Editor)
- **Cards**: `MarkwonFactory.createForCard(context)` MUST use `TaskListPlugin.create(context)`. Never apply `LargeTaskCheckboxDrawable` to cards, as line height on cards (`textAppearanceBodySmall`) is compact, causing large checkboxes to vertically touch and overlap adjacent lines.
- **Editor**: `MarkwonFactory.createForEditor(context)` uses `LargeTaskCheckboxDrawable(context, 16f)`. The drawable's vertical centering accounts for `tv_preview`'s `lineSpacingExtra = 6dp` (`textHeight = b.height() - lineSpacing`), keeping 16dp checkboxes centered relative to the text rather than pushed down into the bottom line spacing.
- **Never override `TaskListItem` AST visitor**: Do not replace Markwon's `TaskListItem` node visitor with custom paragraph spans, as doing so bypasses Markwon's internal list block parser, destroying list indentation, margins, and line breaks.

### 13. Material Design 3 Guidelines
- **Dynamic Colors**: Enabled in `KeepNcApp.onCreate()` via `DynamicColors.applyToActivitiesIfAvailable(this)` for Android 12+ wallpaper palette adaptation.
- **Components**: Use `MaterialToolbar` (with `app:popupTheme="?attr/actionBarPopupTheme"`) and `CircularProgressIndicator` (with `android:indeterminate="true"`) instead of platform views.
- **Accessibility**: All `contentDescription` attributes must use string resources (`@string/cd_*`).
